...
export function Starfield() {
  useEffect(() => {
  const canvasEl = document.getElementById("starfield-canvas") as HTMLCanvasElement | null;
  if (!canvasEl) return;
  ...
  let height = (canvasEl.height = window.innerHeight);
    ...
    const stars: {x:number;y:number;z:number;size:number;s:number;}[] = [];
  ...
  height = canvasEl.height = window.innerHeight;
    ...
    function render(t:number){
      ...
      for (const star of stars){
        ...
        const alpha = Math.min(1, (1000 - star.z)/1000 + 0.2);
