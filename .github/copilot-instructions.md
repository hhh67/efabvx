...
- 本ファイル (`.github/copilot-instructions.md`) は常に最新の運用と実態が乖離しないよう、再読/差分検証を行え。
- 【追加】エラー発生時は必ず `docs/copilot/error-log.md` にフォーマット準拠で直ちに追記し、原因/対処/再発防止策を明文化してから修正コミットを行うこと。修正後は同ログへ「再発防止/Follow-up」項目の進捗を更新。
- 【追加】エラーの再発防止策が新たな規約追加を要する場合、本ファイルへ最小差分で追記し、その後 decision-log.md に反映。
